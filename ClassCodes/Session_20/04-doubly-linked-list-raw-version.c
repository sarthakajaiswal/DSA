...
int insert_after(struct node* head_node, int new_data); 
int insert_before(struct node* head_node, int new_data); 
...
{
    // allocate dummy node 
    // initialize its data, prev and next members to 
    // 0, NULL and NULL respectively 
    // return its address 
...
int remove_start(struct node* head_node) 
{
    ...
    if(remove_node->next != NULL) 
        remove_node->next->prev = head_node; 
...
int remove_end(struct node* head_node) 
{
    ...
    // taking advantage of the information that the run pointer is the last node with data, 
