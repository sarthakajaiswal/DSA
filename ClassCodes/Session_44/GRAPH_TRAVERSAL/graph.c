...
status_t remove_vertex(graph_t* g, vertex_t r_vertex) 
{
    ...
    pv_delete_node = v_search_node(g->pv_head_node, r_vertex);
    if(pv_delete_node == NULL) 
        return (G_INVALID_VERTEX); 

    ph_run = pv_delete_node->ph_head_node->h_next; 
    while(ph_run != pv_delete_node->ph_head_node) 
    {
...
void dfs(graph_t* g) 
{
    ...
    for(pv_start_node = g->pv_head_node->v_next; pv_start_node != g->pv_head_node; pv_start_node = pv_start_node->v_next) 
        if(pv_start_node->color == WHITE) 
            dfs_visit(g, pv_start_node); 
} 
...
{
    pq_mid->q_next = pq_beg; 
    pq_mid->q_prev = pq_end; 
...
{
    if(pv_list->v_prev == pv_list && pv_list->v_next == pv_list) 
        return (TRUE); 
    return (FALSE); 
} 
...
vnode_t* v_get_list_node(vertex_t new_vertex) 
{
    ...
    pv_new_node = xmalloc(sizeof(vnode_t)); 
...
{
    if(ph_list->h_next == ph_list && ph_list->h_prev == ph_list) 
        return (TRUE); 
    return (FALSE); 
} 
